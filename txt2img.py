...
def main():
    ...
    parser.add_argument(
        "--skip_first", # if save the original images, then delete this option in the command line
        action='store_true',
    )
    parser.add_argument(
        "--text_condition",
    ...
    for n in trange(opt.n_iter, desc="Sampling"):
            ...
            for frames_idx, interpolated_condition in enumerate(interpolated_list):
                ...
                if opt.dpm_solver: sampler = DPMSolverSampler(model)
                elif opt.plms: sampler = PLMSSampler(model)
                else: sampler = DDIMSampler(model)
                ...
                if frames_idx != 0:
                    start_code = torch.zeros([opt.n_samples, opt.C, opt.H // opt.f, opt.W // opt.f], device=device)
                    start_code += torch.Tensor(next_first).cuda()
                ...
                with torch.no_grad():
                    ...
                    with precision_scope(True):
                        with model.ema_scope():
                            ...
                            print(f"It is {frames_idx+1} out of {len(interpolated_list)}")
                            for prompts in tqdm(data, desc="data"):
                                uc = None
                                if opt.scale != 1.0:
                                ...
                                c = model.get_learned_conditioning(prompts, [None,0,None]) # 1, 77, 768
                                
                                add_c_list = []
                                for add_cond in add_list: add_c_list.append(add_cond)
                                ...
                                samples_ddim, _ = sampler.sample(S=opt.ddim_steps,
                                                                ...
                                                                adding_cond=add_c_list,
                                                                default_uc=default_uc,
                                                                frames_idx=frames_idx,
                                                                se_value=opt.se_value,
                                ...
                                if not opt.skip_grid: all_samples.append(x_checked_image_torch)
                ...
                frame_list.append(x_sample)
                if frames_idx == 0: next_first = _['x_inter'][n]
                else: next_first = _['x_inter'][0]
                next_start = n # n + 1
